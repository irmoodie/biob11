...
You can download the dataset [here](https://github.com/irmoodie/teaching_datasets/blob/main/tephritis_phenotype/tephritis_phenotype.csv).
...
1. What is the unit of observation in this data set? In other words, what does each row represent?
...
Datasets can often be messy. People make mistakes entering data all the time. You should check this dataset for potential errors. 
For a reference, these flies are very small, less than 1 cm in size.
...
dat <-
tibble(
  x = c(rnorm(99, mean = 200, sd = 4), 220, rnorm(99, mean = 200, sd = 4), 500),
  y = c(rep("Probably not a mistake", times = 100), rep("Probably a mistake", times = 100))
)

a <- ggplot(dat, aes(x = x)) +
facet_wrap(~y, scales = "free") +
geom_histogram(bins = 30)

b <- ggplot(dat, aes(y = x)) +
...
```{r}
tephritis_data |> filter(wing_length_mm > _____) #<1>
```

1. This will only let though rows that have a `wing_length_mm` greater than `>` _____
...
clean_data <- #<1>
  ...
  filter(______ ______ ______) #<2>
```

1. I assigned the cleaned data to a new object, called `clean_data`.
2. You can add more `filter()` functions as you wish.
...
1. What is the overall mean and standard deviation of:
    i) `body_length_mm`
    ii) `ovipositor_length_mm`
    iii) `wing_length_mm`
    iiii) `wing_width_mm`

2. What are the `sex` specific means and standard deviations of:
    i) `body_length_mm`
    iii) `wing_length_mm`
    iiii) `wing_width_mm`

3. What are the `sex` and `host_plant` specific means and standard deviations of:
    i) `body_length_mm`
    ii) `ovipositor_length_mm`
    iii) `wing_length_mm`
    iiii) `wing_width_mm`
...
You will now answer one (or more) of the following questions, by following the steps outlined above and in previous exercises. You should create a dataset that contains only the rows you are interested in, state your hypotheses, make plot(s), and use either a `bootstrap` (as above) or `permute` procedure to test your hypotheses. Write a small text at the end to describe your findings.

To decide which one you will work on, you are going to roll a virtual six sided die! Use this code to do it yourself:

```{r}
cat("You will work on question:", sample(c(1,2,3,4,5,6), 1))
```
...
At the end of this, you should have an idea how you would:

- Import a dataset into R using `read_csv()`.
- Check and clean the dataset of basic errors using `filter()`.
- Produce summary statistics of the dataset using `group_by()` and `summarise()`.
- Produce illustrative plots of the dataset using `ggplot()`
- Compare a mean against a point value, or two means against each other.

# Suggested extra reading:

- Ch. 2, 3 and 4 from @moderndive2025 cover importing, cleaning and plotting data with `tidyverse`.
- Ch. 7, 8 and 9 from @moderndive2025 cover hypothesis testing using randomisation in detail.
- Ch. 15 and 16 of @duthie2025 cover probability and the central limit theorum, which we will cover soon.
- Part 7 of 7 of this [video course](https://biomedical-sciences.ed.ac.uk/experimental-design-and-data-analysis/what-to-do-with-experiments/chapter-3-using-r-basic-concepts-hypothesis-test) covers what a p-value is very clearly.
