...
penguins_modified |>
  ...
  geom_point(
    data = penguins_means |> filter(species != "Gentoo"),
    size = 4,
    shape = 23,
    fill = "red",
    colour = "white"
...
p_values |>
  ...
  labs(
    x = "Number of hypothesis tests needed",
    y = "Probability of finding a p < 0.05 by chance"
  )
```
...
within_plot <-
  ...
  geom_hline(
    data = means,
    aes(yintercept = group_mean, colour = group),
    linetype = "solid"
  ) +
  geom_hline(
    data = means,
